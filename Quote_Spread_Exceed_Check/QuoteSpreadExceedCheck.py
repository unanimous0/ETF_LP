...
sheet_ongoing = book.sheets["1시간 이상 초과"]
            ...
            if row[0] != '':
                ...
                for etf in list(recording_dict.keys()):
                    if etf in comparison_list:
                        ...
                        # TODO 일정 시간동안 초과된 종목을 표시해줬는데, 1시간이 넘으면 1시간 이후의 모든 반복문에서 엑셀에 기록함
                        #      따라서 "1시간 * ongoing_cnt"를 통해 1시간 간격으로만 표시하도록 변경
                        if (((datetime.now() - recording_dict[etf]).seconds)/60) > 60 * ongoing_cnt:      # 기준 스프레드를 초과한 시간이 1시간 이상인 경우
                            ...
                            sheet_ongoing.range(f"D{ongoing_cnt+1}").value = ((datetime.now() - recording_dict[etf]).seconds)/60

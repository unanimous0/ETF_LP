...
class ETFLPTradingValueDF:
    ...
    def __init__(self):
        ...
        self.file_path = tvds.setPath()

        # 초기 데이터프레임 생성 (엑셀파일로부터)
        # self.raw_data_df = tvds.getRawDFfromExcel(self.file_path)

        # 초기 데이터프레임 생성 (데이터베이스로부터)
        self.raw_data_df = tvds.getRawDFfromDB()

        # 중복제거된 거래일자 리스트
        self.date_list = tvds.getTradingDateList(self.raw_data_df)

        # 딕셔너리 {종목코드:종목명} 생성
        self.etf_code_name_dict = tvds.getETFCodeNameDict(self.raw_data_df)

        # raw data 데이터프레임 재구성 및 총거래대금 column 추가 (순매수/순매도대금 아닌 총거래대금)
        self.df_with_7cols = tvds.rebuildRawDF(self.raw_data_df)

        # KB증권을 포함한 주요 회원사명 리스트 생성
        self.lpcompanies = ['KB증권', 'NH투자증권', '신한투자', '메리츠', '키움증권']

        # 주요 회원사 데이터만 추출
        self.main_lpcomp_df = tvds.getMainLPCompDF(self.df_with_7cols, self.lpcompanies)

        # 컬럼과 각 컬럼에 해당하는 데이터가 섞이지 않도록 딕셔너리 생성 (회원사명 순서 바뀌는 오류 방지)
        self.lpcomp_value_dict = tvds.getLPCompValueDict(self.lpcompanies)
        ...
        self.empty_total_value_df = tvds.getEmptyTotalTradingValueDF(self.lpcompanies)
        ...
        self.total_value_df = tvds.fillTotalTradingValueDF(self.date_list, 
    ...
    def returnTotalTradingValueDF(self) -> tuple:
        return (self.date_list, self.total_value_df)

        
if __name__ == "__main__":
    tvdf = ETFLPTradingValueDF()
    date_list, total_value_df = tvdf.returnTotalTradingValueDF()
    ...
    while True:
        try:
            ...
            if search_date_condition == '1':
                ...
                # TODO raw_data에 없는 날짜를 입력할 경우, 그 날 이후 가장 가까운 가능한 날짜부터 조회되도록 해야함 (시작일/종료일 모두)
                condition_df = tvds.makeConditionalDF(date_list=date_list, start_date=input_start_date, end_date=input_end_date, etf_code=input_etf_code, total_value_df=total_value_df)
                ...
                condition_df = tvds.makeConditionalDF(date_list=date_list, start_date=input_start_date, etf_code=input_etf_code, total_value_df=total_value_df, period=period)

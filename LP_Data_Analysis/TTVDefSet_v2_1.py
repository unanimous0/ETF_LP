...
def getRawDF(file_path) -> pd.DataFrame():
...
def fillTotalTradingValueDF(date_list, code_lpcomp_date_value_dict, etf_code_name_dict, lpcomp_value_dict, lpcompanies, empty_total_value_df) -> pd.DataFrame():
    ...
    for date in date_list:
        for etf_code in code_lpcomp_date_value_dict.keys():
            ...
            # empty_total_value_df에 넣기 위한 행을 리스트로 만들기 (만들면 df.append를 통해 그 행을 empty_total_value_df에 추가)
            ...
            raw_append_dict = {
                        empty_total_value_df.columns[0] : date,                                     # 거래일자
                        empty_total_value_df.columns[1] : etf_code,                                 # 종목코드
                        empty_total_value_df.columns[2] : etf_name,                                 # 종목명
                        empty_total_value_df.columns[3] : lpcomp_value_dict[lpcompanies[0]],        # KB증권
                        empty_total_value_df.columns[4] : lpcomp_value_dict[lpcompanies[1]],        # NH투자증권
                        empty_total_value_df.columns[5] : lpcomp_value_dict[lpcompanies[2]],        # 신한투자
                        empty_total_value_df.columns[6] : lpcomp_value_dict[lpcompanies[3]],        # 메리츠
                        empty_total_value_df.columns[7] : lpcomp_value_dict[lpcompanies[4]]         # 키움증권   
                    }
            
            total_value_df = empty_total_value_df.append(raw_append_dict, ignore_index=True)

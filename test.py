...
df_with_7cols = df_with_7cols.loc[main_company_condition]
...
for etf_code in etf_code_name_dict.keys():
    one_etf_df = df_with_7cols[df_with_7cols['종목코드'] == etf_code]
...
def makeConditionalDataFrame(start_date, end_date, etf_code) -> pd.DataFrame():
    ...
    if start_date == end_date:
        _condition_df = total_value_df[(total_value_df['거래일자'] == start_date) & (total_value_df['종목코드'] == etf_code)]
        # _condition_df2 = total_value_df.query("거래일자 == start_date & 종목코드 == etf_code")
    elif start_date < end_date:
        # 위 조건인 경우, 시작일과 종료일을 date_list에서 시작과 끝으로 지정해서 그 사이의 날짜들 다 가져오면 됨
#        start_date_index = date_list.index(start_date)
#        end_date_index = date_list.index(start_date)
#        condition_period_list = date_list[start_date_index : end_date_index + 1]

        _condition_df = total_value_df[(total_value_df['거래일자'] >= start_date) & (total_value_df['거래일자'] <= end_date) & (total_value_df['종목코드'] == etf_code)]
...
#TODO 기간을 시작일과 종료일 입력받을 수도 있게 만들고 & T일로부터 1W, 1M, 1Y 전의 기간도 출력하도록 만들 
...
while True:    
    try:
        ...
        if (len(INPUT_START_DATE) != 8) | (len(INPUT_START_DATE) != 8):
            raise InputValueError()

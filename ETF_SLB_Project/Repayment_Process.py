...
class Repayment_Process:
    ...
    def process_self_repayment(self, df_self_repayment) -> pd.DataFrame():
        ...
        for i, self_rp_row in df_self_repayment.iterrows():
            ...
            if filter_fsc_sum >= self_rp_row["상환신청수량"]:
                ...
                # 3. 상환신청수량이 아예 동일 펀드에 없는 경우 -> 전수량을 동일 운용사, 다른 펀드에서 상환 (또는 차입 후 상환)
                    ...
                    print(f"{self_rp_row['종목명']} 은(는) 동일 펀드 내에 수량이 1주도 없으므로 상환이 불가능합니다. 동일 운용사, 다른 펀드에서 나머지 상환가능수량을 계속 탐색합니다.\n")
        ...
        print("================================== 동일 펀드에서 상환하고 난 후의 자체상환 신청내역 =================================")
        ...
        list_other_funds_rp, df_self_rp_error = self.process_other_funds_repayment(df_self_rp_other_funds)
        for _dict in list_other_funds_rp:
            input_list.append(_dict)
        
        print("============================ 동일 운용사, 다른 펀드에서 상환하고 난 후의 자체상환 신청내역 ===========================")
        print(df_self_rp_other_funds)
        print("===================================================================================================================\n")
        
        print("================================= Matched Table에서 상환 불가능한 자체상환 신청내역 =================================")
    ...
    def process_other_funds_repayment(self, df_self_rp_other_funds) -> dict():
        ...
        for i, self_rp_row in df_self_rp_other_funds.iterrows():
            ...
            if filter_fsc_sum >= self_rp_row["상환신청수량"]:
                ...
                # 3. 상환신청수량이 아예 동일 펀드에 없는 경우 -> 전수량을 동일 운용사, 다른 펀드에서 상환 (또는 차입 후 상환)
                    ...
                    print(f"{self_rp_row['종목명']} 은(는) 동일 운용사 내에 수량이 1주도 없으므로 상환이 불가능합니다. Unmatched Table을 탐색하거나 다른 운용사에서 차입한 내역을 확인하세요. (또는 차입후 상환)\n")
        ...
        print("==================================== 동일 운용사, 다른 펀드에서 상환 가능한 내역 ====================================")
        print(other_funds_repayment)
        print("===================================================================================================================\n")
        
        return (input_list, df_self_rp_error)
